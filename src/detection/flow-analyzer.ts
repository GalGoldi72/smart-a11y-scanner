...
export class FlowAnalyzer implements IFlowAnalyzer {
   ...
   */
  async analyzeStructure(page: Page, url: string): Promise<PageFlowAnalysis> {
  ...
  private async detectNavigations(page: Page): Promise<NavigationStructure[]> {
    const raw: Array<{ pattern: string; containerSelector: string; itemCount: number; landmark: string | null }> = await page.evaluate(() => {
      const results: any[] = [];
      ...
      for (const nav of navElements) {
        ...
        if (rect.width < 300 && rect.height > 400) pattern = 'sidebar-nav';
        if (rect.top > document.documentElement.scrollHeight * 0.8) pattern = 'footer-nav';
      ...
      const breadcrumbs = document.querySelectorAll(
        '[aria-label*="breadcrumb" i], [role="navigation"][aria-label*="breadcrumb" i], .breadcrumb, .breadcrumbs, nav.breadcrumb',
      );
      ...
      const paginations = document.querySelectorAll(
        'nav[aria-label*="paginat" i], .pagination, [role="navigation"][aria-label*="paginat" i]',
      );
    ...
    return raw.map((r) => ({
      pattern: r.pattern as NavigationStructure['pattern'],
  ...
  private async detectForms(page: Page, elements: ElementInfo[]): Promise<FormInfo[]> {
    const rawForms = await page.evaluate(() => {
      ...
      for (const form of document.querySelectorAll('form')) {
        ...
        const allFieldNames = fields
          .map((f) =>
            [f.getAttribute('name'), f.getAttribute('id'), f.getAttribute('autocomplete'), f.getAttribute('type')]
              .filter(Boolean)
              .join(' '),
          )
          .join(' ')
        ...
        let purpose = 'generic';
        if (/password|login|sign.?in|auth/i.test(actionStr)) purpose = 'login';
        else if (/search/i.test(actionStr)) purpose = 'search';
        else if (/register|sign.?up|create.?account/i.test(actionStr)) purpose = 'registration';
        else if (/contact|message|feedback/i.test(actionStr)) purpose = 'contact';
        else if (/subscribe|newsletter/i.test(allFieldNames)) purpose = 'newsletter';
        else if (/payment|card|billing/i.test(allFieldNames)) purpose = 'payment';
  ...
  private async detectTriggers(page: Page, elements: ElementInfo[]): Promise<TriggerRelationship[]> {
    const rawTriggers = await page.evaluate(() => {
      ...
      for (const el of document.querySelectorAll('[aria-controls]')) {
        ...
        let action = 'toggles';
        if (hasPopup === 'dialog' || hasPopup === 'true') action = 'opens-modal';
        else if (hasPopup === 'menu' || hasPopup === 'listbox') action = 'opens-dropdown';
        else if (expanded !== null) action = 'expands-section';
      ...
      for (const el of document.querySelectorAll('[data-toggle], [data-bs-toggle]')) {
        ...
        let action = 'toggles';
        if (toggle === 'modal') action = 'opens-modal';
        else if (toggle === 'dropdown') action = 'opens-dropdown';
        else if (toggle === 'collapse') action = 'expands-section';
    ...
    const stubElement = (selector: string): ElementInfo => ({
      selector,
      tag: 'unknown',
    ...
    return rawTriggers.map((raw: any) => ({
  ...
  async traceTabOrder(page: Page): Promise<TabOrderNode[]> {
    const focusableElements = await this.detector.detectFocusable(page);

    const orderedSelectors: string[] = await page.evaluate(() => {
      function buildSelector(el: Element): string {
        if (el.id) return `#${CSS.escape(el.id)}`;
        const parts: string[] = [];
        let current: Element | null = el;
  ...
  private async detectLandmarks(
    page: Page,
  ): Promise<{ role: string; selector: string; label: string | null }[]> {
    return page.evaluate(() => {
      ...
      for (const [tag, role] of Object.entries(implicitMap)) {
        for (const el of document.querySelectorAll(tag)) {
          ...
          if ((tag === 'section' || tag === 'form') && !el.getAttribute('aria-label') && !el.getAttribute('aria-labelledby')) continue;
          if ((tag === 'header' || tag === 'footer') && el.closest('article, aside, nav, section')) continue;
          landmarks.push({ role, selector: buildSelector(el), label: el.getAttribute('aria-label') ?? null });
        }
      }
   ...
   */
  private async buildTabFlow(page: Page, url: string): Promise<UserFlow | null> {
    ...
    if (outOfOrder.length > 0) {
      issues.push(
        `${outOfOrder.length} element(s) appear out of visual order in the tab sequence`,
      );
    }
    ...
    return {
      ...
      interactions: tabOrder.map((node) => ({
        ...
        interactionType: 'keyboard' as const,
        success: true,
        error: null,
        domDelta: {
          addedSelectors: [],
