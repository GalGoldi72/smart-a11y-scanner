...
export class SiteMapper {
  ...
  private simulator: InteractionSimulator;
   ...
   */
  async buildGraph(context: BrowserContext, seedUrl: string): Promise<SiteGraph> {
    ...
    while (this.urlQueue.length > 0 && depth <= this.config.maxCrawlDepth) {
      ...
      for (const url of currentBatch) {
        if (this.visitedUrls.has(this.normalizeUrl(url))) continue;
  ...
  private async analyzePage(
    page: Page,
    url: string,
    discoveredVia: SitePageNode['discoveredVia'],
  ): Promise<SitePageNode> {
    // Navigate
    ...
    // Run flow analysis
    let flowAnalysis: PageFlowAnalysis | null = null;
    try {
    ...
    const forms: FormInfo[] = flowAnalysis?.forms ?? [];
   ...
   */
  private async discoverInteractionEdges(
    page: Page,
    sourceUrl: string,
   ...
   */
  private extractLinks(pageNode: SitePageNode, seedUrl: string): string[] {
    const links: string[] = [];
   ...
   */
  private isSameOrigin(url: string, seedUrl: string): boolean {
    try {
      const a = new URL(url);
