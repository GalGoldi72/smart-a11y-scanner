...
export class TestCaseImporter {
    ...
    async listSuites() {
        ...
        do {
            const resp = await this.testPlanHttp.get(`/plans/${this.config.testPlanId}/suites`, { params: { continuationToken, $top: PAGE_SIZE } });
    ...
    async resolveSuites() {
        ...
        if (targetIds && targetIds.length > 0) {
            ...
            if (matched.length === 0) {
                this.warnings.push(`None of the requested suite IDs [${targetIds.join(', ')}] were found in plan ${this.config.testPlanId}.`);
            }
            return matched;
        }
        return allSuites;
    }
    async fetchTestCasesFromSuites(suites) {
        const cases = [];
        const seenIds = new Set();
        for (const suite of suites) {
            let continuationToken;
            do {
                const resp = await this.testPlanHttp.get(`/plans/${this.config.testPlanId}/suites/${suite.id}/testcase`, { params: { continuationToken } });
                const items = resp.data.value ?? [];
        ...
        for (let i = 0; i < ids.length; i += PAGE_SIZE) {
            ...
            for (const item of items) {
                try {
                    ...
                    this.warnings.push(`Failed to parse work item ${item.id}: ${err instanceof Error ? err.message : String(err)}`);
                }
            }
        }
        return results;
    }
    mapWorkItemFields(item) {
        const f = item.fields;
    ...
    applyFilters(items) {
        ...
        return items.filter(tc => {
            if (filter.states && filter.states.length > 0) {
                if (!filter.states.includes(tc.state)) {
                    return false;
                }
            }
    ...
    parseTestCase(tc, suiteInfo) {
        ...
        for (const step of tc.steps) {
            ...
            if (step.expectedResultText.trim()) {
                ...
                if (behavior)
                    expectedBehaviors.push(behavior);
            }
        }
    ...
    parseExpectedBehavior(text) {
        if (!text.trim())
            return null;
        const behavior = { description: text };
    ...
    // Suite lookup helper
    // -------------------------------------------------------------------------
    findSuiteForTestCase(testCaseId, rawCases, suites) {
...
//# sourceMappingURL=test-case-importer.js.map