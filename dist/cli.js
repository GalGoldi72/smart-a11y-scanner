...
class ProgressDisplay {
    ...
    onPageComplete(url, findingsCount, timeMs) {
        this.pagesScanned++;
        ...
        if (this.verbose) {
            ...
            process.stdout.write(`\r  ${chalk.cyan('⟳')} ${this.pagesScanned} pages scanned | ${this.totalFindings} findings | ${elapsed}s`);
        }
    }
    finish() {
        if (!this.verbose) {
...
function printSummary(result) {
    ...
    console.log('');
    // Severity breakdown
    const sevOrder = ['critical', 'major', 'minor', 'advisory'];
