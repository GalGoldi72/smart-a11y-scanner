...
export class InteractionSimulator {
     ...
     */
    async focus(page, element) {
        const before = await this.takeSnapshot(page);
        try {
            await page.focus(element.selector, { timeout: this.config.timeout });
     ...
     * or hit a maximum.
     */
    async tabThrough(page, maxSteps = 100) {
        const results = [];
        let firstFocusedSelector = null;
     ...
     */
    async interactWithAll(page, elements) {
        const results = [];
    ...
    async takeSnapshot(page) {
        const data = await page.evaluate(() => {
            ...
            const significantElements = document.querySelectorAll('a, button, input, select, textarea, [role], img, h1, h2, h3, h4, h5, h6, form, ' +
            ...
            const hasVisibleModal = document.querySelector('dialog[open]') !== null ||
    ...
    computeDelta(before, after) {
        ...
        for (const sel of after.selectors) {
            if (!before.selectors.has(sel))
                addedSelectors.push(sel);
        }
        for (const sel of before.selectors) {
            if (!after.selectors.has(sel))
     ...
     */
    buildResult(element, type, success, error, before, after) {
        return {
            element,
    ...
    guessInputType(element) {
        ...
        if (combined.includes('url') || combined.includes('website'))
            return 'url';
        if (combined.includes('number') || combined.includes('quantity'))
            return 'number';
        return 'text';
    }
}
//# sourceMappingURL=interaction-simulator.js.map